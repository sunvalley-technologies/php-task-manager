...
use WyriHaximus\React\ChildProcess\Messenger\StaticConfig;

/** @internal Copied from WyriHaximus to expose process to messenger for sending signals to child */
final class ProcessFactory
{
    ...
    public static function parent(
        ...
        return new Promise\Promise(
            function ($resolve, $reject) use ($process, $loop, $options) {
    ...
    public static function parentFromClass(
        ...
        return new Promise\Promise(
            function ($resolve, $reject) use ($class, $loop, $options) {
                $server             = new Server('127.0.0.1:0', $loop);
                $options['random']  = \bin2hex(\random_bytes(32));
                $options['address'] = $server->getAddress();
                ...
                if (isset($options['childProcessPath'])) {
                    $childProcessPath = $options['childProcessPath'];
                    unset($options['childProcessPath']);
                } else {
                    $sp               = DIRECTORY_SEPARATOR;
                    $childProcessPath = \escapeshellarg(
                        __DIR__ . $sp . '..' . $sp . 'bin' . $sp . 'php-task-manager-worker.php'
                    );
                }
                ...
                \WyriHaximus\React\futurePromise($loop)->then(
                    function () use ($process, $server, $loop, $options, $connectTimeout) {
                        return Promise\Timer\timeout(
                            self::startParent($process, $server, $loop, $options),
                            $connectTimeout,
                            $loop
                        );
                    }
                )->then(
                    function (Messenger $messenger) use ($class) {
                        return $messenger->rpc(
                            MessengesFactory::rpc(
                                \WyriHaximus\React\ChildProcess\Messenger\Factory::PROCESS_REGISTER,
                                [
                                    'className' => $class,
                                ]
                            )
                        )->then(
                            function ($p) use ($messenger) {
    ...
    public static function child(LoopInterface $loop, array $options = [], callable $termiteCallable = null)
    {
        ...
        return (new Connector($loop, ['timeout' => $connectTimeout]))->connect($options['address'])->then(
            function (ConnectionInterface $connection) use ($options, $loop, $connectTimeout) {
                return new Promise\Promise(
